...
fn dbsnp_release_info() -> Result<String> {
    let version_dl = Download::new(RELEASEFILE.to_string(), 
                                        DBSNPSERVER.to_string(), 
                                        LOCALPATH.to_string());
    let release_notes = version_dl.get_text()?;
    Ok(release_notes)
}

pub fn dbsnp_latest_release() -> Result<String> {
    let release_info = dbsnp_release_info()?;
    let version = release_info.split_whitespace().nth(VERSIONLOC).unwrap_or("NA");
...
fn download_check_file(dl: Download, dl_md5: Download, force: bool) -> Result<()> {
    let md5_info = dl_md5.get_text()?;
    ...
    let md5_calc_hash = md5_hash_file_verbose(dl.localfile().unwrap())?;
    assert_eq!(md5_dl_hash, md5_calc_hash);
    println!("Successfully validated file");
    Ok(())
}
pub fn download_dbsnp(force: bool) -> Result<()> {
    ...
    let dbsnp_version = dbsnp_latest_release()?;
